...
// Crash-proof wrapper for the signed contracts section
    ...
    if (this.state.hasError) {
      return (
        <div className="p-6 text-center">
          <div className="inline-flex items-center gap-2 rounded-lg bg-rose-50 border border-rose-200 px-4 py-3 text-sm text-rose-700">
            <XCircleIcon className="h-4 w-4 flex-shrink-0" />
            <span>{this.props.fallbackLabel ?? 'Đã xảy ra lỗi. Vui lòng làm mới trang.'}</span>
          </div>
        </div>
      );
    }
...
function buildReportInsights(data: ReportsSummary): ReportInsight[] {
  ...
  if (data.expiring_30d_count >= 5) {
    ...
    insights.push({
      ...
      description: data.expiring_30d_count > 0
        ? `Trong đó ${data.expiring_30d_count} hết trong 30 ngày — cần xử lý sớm.`
        : 'Cân nhắc kế hoạch theo dõi và chuẩn bị tái ký.',
    });
  }
...
function getSignedScopeRange(scope: SignedScope, now: Date) {
  ...
  if (scope === 'week') {
    ...
    return { start, end, label: `Tuần này: ${formatDateRangeLabel(start, end)}` };
  }
  if (scope === 'month') {
    const start = new Date(base.getFullYear(), base.getMonth(), 1);
    const end = new Date(base.getFullYear(), base.getMonth() + 1, 0);
    return { start, end, label: `Tháng này: ${formatDateRangeLabel(start, end)}` };
  }
  if (scope === 'quarter') {
    ...
    return { start, end, label: `Quý này: ${formatDateRangeLabel(start, end)}` };
  }
  const start = new Date(base.getFullYear(), 0, 1);
  const end = new Date(base.getFullYear(), 11, 31);
  return { start, end, label: `Năm nay: ${formatDateRangeLabel(start, end)}` };
}
...
export function ReportsPage({
  ...
  const fetchSignedContracts = React.useCallback(async () => {
    ...
    try {
      ...
      if (signedScope === 'year') {
        requestParams.scope = 'year';
        requestParams.year = signedYearFilter || undefined;
      } else {
        const range = getSignedScopeRange(signedScope, new Date());
        requestParams.date_from = toIsoDate(range.start);
        requestParams.date_to = toIsoDate(range.end);
      }

      const result = await listSignedContracts(token, requestParams);
  ...
  const signedSummary = useMemo(() => {
    ...
    const totalValue = signedContracts?.total_value ?? 0;
    const avg = signedContracts?.average_value ?? 0;
    return { count, totalValue, avg };
  }, [signedContracts]);

  const signedRange = useMemo(() => getSignedScopeRange(signedScope, today), [signedScope, today]);

  const signedRangeLabel = useMemo(() => {
    if (signedContracts?.applied_date_from && signedContracts?.applied_date_to) {
      const start = new Date(`${signedContracts.applied_date_from}T00:00:00`);
      const end = new Date(`${signedContracts.applied_date_to}T00:00:00`);
      const scopeLabel = signedContracts.applied_scope === 'week'
        ? 'Tuần này'
        : signedContracts.applied_scope === 'month'
          ? 'Tháng này'
          : signedContracts.applied_scope === 'quarter'
            ? 'Quý này'
            : signedContracts.applied_scope === 'year'
              ? 'Năm nay'
              : 'Khoảng áp dụng';
      return `${scopeLabel}: ${formatDateRangeLabel(start, end)}`;
    }
    return signedRange.label;
  }, [signedContracts, signedRange]);
    ...
    <Page>
          ...
          <MetricStrip
            items={[
              ...
              },
              {
                ...
                tone: 'sky',
                muted: stats.totalWorks === 0,
                icon: <Music2Icon className="h-4 w-4" />,
                hint: stats.totalWorks > 0 ? 'Đang quản lý' : 'Chưa có dữ liệu tác phẩm',
              },
              {
                label: 'GCN bản nháp',
      ...
      {sectionVis.signed && dataTab === 'signed' && (
        <div key="tab-signed" className="tab-swap">
          ...
          >
            <ErrorBoundary fallbackLabel="Không tải được dữ liệu hợp đồng đã ký. Vui lòng làm mới trang.">
              {/* Summary strip */}
              <div className="px-5 py-3 border-b border-zinc-100/80 bg-zinc-50/40">
                <div className="inline-flex items-center rounded-full bg-amber-100 px-3 py-1 text-[12px] font-medium text-amber-800">
                  {signedRangeLabel}
                </div>
              </div>
              ...
              {signedContracts === null ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="sticky top-0 z-10 bg-gradient-to-b from-amber-50/30 via-zinc-50 to-zinc-50/30 border-b border-zinc-200">
                        ...
                        <th className="px-3 py-3 text-left text-xs font-semibold text-zinc-500 uppercase tracking-wide whitespace-nowrap w-px">Ngày ký</th>
                        <th className="px-4 py-3 text-left text-xs font-semibold text-zinc-500 uppercase tracking-wide">Đơn vị / Bảng hiệu</th>
                        <th className="px-3 py-3 text-left text-xs font-semibold text-zinc-500 uppercase tracking-wide whitespace-nowrap w-px">Lĩnh vực</th>
                        <th className="px-4 py-3 text-right text-xs font-semibold text-zinc-500 uppercase tracking-wide whitespace-nowrap">Giá trị</th>
                        <th className="px-3 py-3 text-left text-xs font-semibold text-zinc-500 uppercase tracking-wide whitespace-nowrap w-px">Thời hạn</th>
                        <th className="px-3 py-3 text-left text-xs font-semibold text-zinc-500 uppercase tracking-wide whitespace-nowrap w-px">GCN</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-zinc-100">
                      {signedRows.map((r) => (
                        <tr
                          ...
                          <td className="px-3 py-3.5 align-top tabular-nums text-[13px] whitespace-nowrap text-zinc-700">
                            {r.signed_date ? formatDate(r.signed_date) : '—'}
                          </td>
                          <td className="px-4 py-3.5 align-top max-w-[340px]">
                            <p className="text-[14px] font-semibold text-zinc-900 leading-snug line-clamp-2" title={r.partner ?? undefined}>
                              {r.partner ?? '—'}
                            </p>
                            {r.brand && (
                              <p className="mt-0.5 text-[12px] text-zinc-500 truncate" title={r.brand}>{r.brand}</p>
                            )}
                          </td>
                          <td className="px-3 py-3.5 align-top text-[13px] text-zinc-700">
                            {r.field ?? '—'}
                          </td>
                          <td className="px-4 py-3.5 align-top text-right tabular-nums whitespace-nowrap">
                            {r.value == null ? (
                          ...
                          <td className="px-3 py-3.5 align-top">
                            {r.gcn_certificate_no ? (
                              <span className="inline-flex items-center gap-1.5 text-[12px] font-mono text-zinc-700">
                                <span className={`inline-block w-1.5 h-1.5 rounded-full ${
                                  r.gcn_status === 'final_printed' ? 'bg-green-500' :
                                  r.gcn_status === 'test_printed' ? 'bg-amber-500' :
                                  'bg-zinc-400'
                                }`} />
                                {r.gcn_certificate_no}
                              </span>
                  ...
                  {signedContracts.total > SIGNED_PAGE_SIZE && (
                    <div className="flex items-center justify-between px-5 py-3 border-t border-zinc-100 bg-zinc-50/40 text-[12px]">
                      <span className="text-zinc-600 tabular-nums">
                        Trang <span className="font-semibold text-zinc-900">{signedCurrentPage}</span> / {signedTotalPages} · {signedContracts.total} hợp đồng
                      </span>
                ...
                {pendingRows.map((p) => {
                  ...
                  return (
                    <tr
                      ...
                      <td className="px-4 py-3.5 align-top text-right tabular-nums whitespace-nowrap text-[13px] text-zinc-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <ContentCard
              ...
              <BarChart
                ...
                <defs>
                  ...
                  <linearGradient id="rep2BarFillPrev" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#b9e2c2" stopOpacity={0.85} />
                ...
                <Tooltip
                  ...
                  contentStyle={{
                    ...
                    borderRadius: 12,
                    background: 'rgba(22, 50, 32, 0.94)',
                    backdropFilter: 'blur(8px)',
                    color: '#fff',
                    fontSize: 12,
                    padding: '8px 12px',
                    boxShadow: '0 14px 40px rgba(22,163,74,0.2)',
                  }}
                  labelStyle={{ color: '#86efac', fontWeight: 600, marginBottom: 2 }}
                  itemStyle={{ color: '#fff' }}
          ...
          <p className="mt-3 text-[11px] text-zinc-500 leading-relaxed border-t border-zinc-100 pt-3">
            Năm hiện tại là dữ liệu lũy kế đến hôm nay. Đường gạch nét là <span className="font-semibold text-green-700">dự báo cuối năm</span> dựa trên tốc độ hiện tại ({(yearForecast?.progress ? yearForecast.progress * 100 : 0).toFixed(0)}% năm đã qua). Cột pattern gạch chéo nghĩa là chưa có dữ liệu.
          </p>
        </ContentCard>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="sticky top-0 z-10 bg-gradient-to-b from-amber-50/30 via-zinc-50 to-zinc-50/30 border-b border-zinc-200">
                ...
                <Th className="w-px">Số hợp đồng</Th>
                <Th>Đơn vị</Th>
                <Th>Trạng thái</Th>
                ...
                <tr
                  ...
                  <td className="px-3 py-3.5 align-top whitespace-nowrap">
                    {c.certificate_no ? (
                      ...
                      <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-medium bg-amber-50 text-amber-700 ring-1 ring-inset ring-amber-600/15">
                        Chưa cấp số
                      </span>
                    )}
                  </td>
                  ...
                  <td className="px-4 py-3.5 align-top max-w-[320px]">
                    <p className="text-[13px] font-semibold text-zinc-900 leading-snug line-clamp-2" title={c.organization_name ?? undefined}>
                      {c.organization_name || '—'}
                    </p>
                  </td>
                  <td className="px-3 py-3.5 align-top">
                  ...
                  <td className="px-3 py-3.5 align-top text-right tabular-nums whitespace-nowrap">
                    {c.print_count > 0 ? (
        ...
        {tabs.map((t) => {
          ...
          return (
            <button
              ...
              className={`group relative min-w-[120px] px-4 py-3 rounded-xl text-left transition-all duration-200 ease-out ${
                isActive
...
function Th({
  ...
  return (
    <th
      className={`px-4 py-3.5 text-[11px] font-semibold uppercase tracking-[0.08em] text-zinc-700 ${className} ${
    ...
    <div className="rounded-xl ring-1 ring-zinc-900/[0.06] bg-white p-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          ...
          <p className="text-base font-semibold text-zinc-600 tabular-nums">
            {formatNumber(count)}
          </p>
        </div>
        <div>
